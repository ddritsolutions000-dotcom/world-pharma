...
describe('R7-C sample collection + chain of custody (e2e)', () => {
  ...
  it('enqueues collection, CoC lifecycle, isolation, append-only, PHI minimization', async () => {
    ...
    const admin = await signIn(app, `r7c-admin-${suffix}@example.com`, 'admin');
    const labUser = await signIn(app, `r7c-la-${suffix}@example.com`);
    const otherLab = await signIn(app, `r7c-lb-${suffix}@example.com`);
    const customerA = await signIn(app, `r7c-ca-${suffix}@example.com`);
    const customerB = await signIn(app, `r7c-cb-${suffix}@example.com`);
    const pheA = await signIn(app, `r7c-phe-a-${suffix}@example.com`);
    const pheB = await signIn(app, `r7c-phe-b-${suffix}@example.com`);
    const outsider = await signIn(app, `r7c-outsider-${suffix}@example.com`);
