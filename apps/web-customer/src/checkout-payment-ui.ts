export function checkoutPayButtonLabel(
  ...
  switch (method) {
    ...
    case 'MOBILE_PAYMENT':
      return country?.toUpperCase() === 'IN' ? 'Pay with UPI' : 'Pay with mobile payment';
    case 'WALLET':
...
export function checkoutSuccessCopy(method: string, country?: string): { title: string; description: string } {
  if (method === 'COD') {
    return {
      title: 'Order placed successfully',
      description: 'Pay in cash when your order is delivered. Track status from My Orders.',
    };
  }
  if (method === 'MOBILE_PAYMENT') {
    const upi = country?.toUpperCase() === 'IN';
    return {
      title: 'Payment successful',
      description: upi
        ? 'Your UPI payment was confirmed and your order is being processed.'
        : 'Your mobile payment was confirmed and your order is being processed.',
    };
  }
  if (method === 'WALLET') {
    return {
...
export function paymentMethodHint(method: string, country: string): string | null {
  ...
  if (code === 'IN') {
    ...
    if (method === 'MOBILE_PAYMENT') {
      return 'You will complete payment in your UPI app (PhonePe, Google Pay, Paytm).';
    }
  }
  ...
  if (code === 'AE' && method === 'MOBILE_PAYMENT') {
    return 'Complete payment with your mobile wallet provider.';
  }
  if (code === 'US' && method === 'MOBILE_PAYMENT') {
    return 'Complete payment with your mobile payment provider.';
  }
  return null;
}
...
export function isUpiCollectPending(
  intent: { status?: string; next_action?: { type?: string; vpa?: string } } | null | undefined,
  method: string,
): boolean {
  return (
