...
export function storeOrderNextActionLabel(status: string): string | null {
  const action = storeOrderNextAction(status);
  if (action === 'pick/start') return 'Start pick';
  if (action === 'pick/complete') return 'Complete pick';
  if (action === 'pack/complete') return 'Complete pack';
  if (action === 'ready') return 'Ready to ship';
  return null;
}
