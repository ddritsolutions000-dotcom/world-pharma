-- R8-F: extend REPORT_DELIVERY logistics RLS for imaging physical report requests.
...
CREATE POLICY logistics_jobs_access ON logistics_jobs TO worldpharma_app
  USING (
    ...
    OR (
      "job_type" = 'REPORT_DELIVERY'
      AND "imaging_physical_report_request_id" IS NOT NULL
      AND (
        app.is_worker()
        OR (
          app.person_id() IS NOT NULL
        ...
        OR EXISTS (
          SELECT 1 FROM imaging_physical_report_requests ipr
          WHERE ipr.id = logistics_jobs.imaging_physical_report_request_id
            AND (
              app.can_org(ipr.imaging_org_id)
              OR app.can_person(ipr.customer_person_id)
            )
        )
      )
    )
