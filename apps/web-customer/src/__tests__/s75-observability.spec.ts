/**
 * Sprint 75 — Observability / APM readiness evidence (no fake APM vendor).
...
import {
  ...
  selectMarketIfGated,
  uiOtpLogin,
} from '../../e2e/helpers/s75-ui';

test.beforeEach(async () => {
  await clearOtpRateLimits();
});

test.describe('S75 observability API gates', () => {
  ...
  test('health echoes correlation id; metrics endpoint present', async ({ request }) => {
    const corr = 's75-corr-health-probe';
    ...
    // With or without METRICS_TOKEN: 200 text or 401 — never leak secrets
  ...
  test('controlled unsigned webhook failure is fail-closed', async ({ request }) => {
    const corr = 's75-webhook-fail';
    const wh = await request.post('http://127.0.0.1:4000/api/v1/webhooks/payments/MOCK_PRIMARY', {
      headers: { 'x-correlation-id': corr },
      data: { event_id: 's75-unauth-fail', type: 'payment.captured', note: 'sandbox controlled failure' },
    });
...
test.describe('S75 Admin + controlled failure recovery', () => {
  test('APM NOT_SELECTED + failure visibility + customer recovery', async ({ page, context, browser }) => {
    ...
    const obsHeading = page
      .getByText(/Observability \/ APM \/ alerting (readiness|activation readiness)/i)
      .first();
    ...
    await ensureNoSecrets(page);
    await s75Snap(page, 'admin-01-observability-status');
    await s75Snap(page, 'admin-02-apm-external-gated');
    ...
    await s75Snap(page, 'admin-03-reliability-signals');
    ...
    await s75Snap(page, 'admin-04-launch-readiness');

    // Recovery: customer can still authenticate after controlled webhook failure
    ...
    await s60AdminLogin(page);
    await page.goto(`${ADMIN}/provider-activation`, { waitUntil: 'domcontentloaded' });
    const finalGate = page.getByText(/NO_PRODUCTION_APM_PROVIDER|Observability \/ APM/i).first();
    await expect(finalGate).toBeVisible({ timeout: 60_000 });
    await finalGate.scrollIntoViewIfNeeded();
    await s75Snap(page, 'admin-07-final-status');
    ...
    s75WriteArtifact('final-observability-status.json', JSON.stringify(status, null, 2));
    await s75Snap(page, 'admin-08-final-context');
  });
});
