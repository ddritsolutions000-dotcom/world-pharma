...
export function matchAddressForMember(
  member: FamilyMember,
  addresses: Array<{ id: string; recipient_name?: string; recipientName?: string }>,
): string | null {
  const target = member.display_name.trim().toLowerCase();
  const hit = addresses.find((row) => {
    const name = (row.recipient_name ?? row.recipientName ?? '').trim().toLowerCase();
    return name === target;
  });
  return hit?.id ?? null;
}
