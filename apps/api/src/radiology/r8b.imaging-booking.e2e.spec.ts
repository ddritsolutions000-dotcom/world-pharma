...
describe('R8-B customer imaging booking + sandbox pay (e2e)', () => {
  ...
  it('discovers, books, pays sandbox, isolates tenants/locations, fail-closed packs', async () => {
    ...
    const admin = await signIn(app, `r8b-admin-${suffix}@example.com`, 'admin');
    const imagingUser = await signIn(app, `r8b-ia-${suffix}@example.com`);
    const otherImaging = await signIn(app, `r8b-ib-${suffix}@example.com`);
    const customerA = await signIn(app, `r8b-ca-${suffix}@example.com`);
    const customerB = await signIn(app, `r8b-cb-${suffix}@example.com`);
