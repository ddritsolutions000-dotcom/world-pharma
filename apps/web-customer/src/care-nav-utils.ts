...
export function isEditableCareNavStatus(status: CareNavSessionStatus): boolean {
...
export function matchExplanationCopy(key: string): string {
  ...
  const map: Record<string, string> = {
    ...
    'care_nav.match.tele_capable': 'This provider supports online consultation when enabled for your region.',
  ...
  if (map[primary]) {
    return key.includes('tele_capable')
      ? `${map[primary]} Online consultation may be available for some providers.`
      : map[primary]!;
  }
  return 'Provider recommendation based on operational eligibility rules.';
}
